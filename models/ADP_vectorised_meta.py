...
def train(train_ts_vectorised, test_ts_vectorised, meta_ts, S, Q, rng, learning_rate, n_iters):
    ...
    for epoch in range(n_iters):
        ...
        ws_vector_test = torch.index_select(ws, 0, test_ts_vectorised[1])
        meta_vector_test = torch.index_select(meta_ts, 0, test_ts_vectorised[2])
        ps_vector_test = ws_vector_test*meta_vector_test

        probit_1_test = 1/(1+torch.exp(-bs_vector_test-bq_vector_test-ps_vector_test))
    ...
    return bs, bq, ws, n_iters, nll_train_arr, nll_test_arr


def predict(bs, bq, ws, test_ts, meta_ts, rng, held_out_test):
    ...
    product_params_matrix = probit_correct(bs_vector, bq_vector, ps_vector)
...
def train_product_meta(ts, df, meta_ts, rng, learning_rate, n_iters, held_out_test):
    ...
    bs, bq, ws, n_iters, nll_train_arr, nll_test_arr = train(train_vectorised_ts, test_vectorised_ts, meta_ts, S, Q, rng, learning_rate, n_iters)
    ...
    product_params_matrix, performance, conf_matrix = predict(bs, bq, ws, test_vectorised_ts, meta_ts, rng, held_out_test)

    print(f"bs (student params): {bs}")
    print(f"bq (question params): {bq}")
    print(f"ws (problem solving student params): {ws}")
