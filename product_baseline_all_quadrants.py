...
def train(learning_rate, n_iters, upper_half_ts, left_half_ts):

    t_arr, nll_arr = [], []
    S = left_half_ts.shape[0] # no. of rows
    Q = upper_half_ts.shape[1] # no. of cols
    ...
    for epoch in range(n_iters):
        ...
        bs_matrix = bs_tensor.repeat(upper_half_ts.shape[1], 1)
        bs_matrix = torch.transpose(bs_matrix, 0, 1)
        bq_matrix = bq_tensor.repeat(upper_half_ts.shape[0], 1)
